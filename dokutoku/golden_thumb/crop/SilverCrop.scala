...
class SilverCrop(id: Int, germ: Int) extends GoldenCrop(id, germ) {
  ...
  override def randomDisplayTick(world: World, x: Int, y: Int, z: Int, rand: Random) : Unit = {
    ...
    if(randu == 1) metalFXEffect(world, x, y, z, rand, 2.00F, 2.20F, 2.27F);
	else           metalFXEffect(world, x, y, z, rand, 0.72F, 1.40F, 1.46F);
  }
}